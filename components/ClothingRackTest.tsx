...
const ClothingRack: FC<Props> = ({data, onSelect }) => {
    const RackButton = useRef();
  const [visible, setVisible] = useState(false);
  const [selected, setSelected] = useState(undefined);
//   const [dropdownTop, setDropdownTop] = useState(0);

  const toggleRack = (): void => {
    visible ? setVisible(false) : openRack();
  };
  ...
  const openRack = (): void => {
    ...
    const onItemPress = (items: React.SetStateAction<undefined>): void => {
      setSelected(item);
      onSelect(item);
      setVisible(false);
  }
  ...
  const renderRack = (): ReactElement<any, any> => {
    if(visible){
        return(
  ...
  return(
    <View>
        <TouchableOpacity onPress={toggleRack} style={containerStyle.button}>
         <Text style={containerStyle.buttonText}>Clothing Rack</Text>
        </TouchableOpacity>
        {renderRack()}
    </View>
